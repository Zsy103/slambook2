...
class CurveFittingVertex : public g2o::BaseVertex<5, Eigen::Matrix< double, 5, 1 >> // 5: 优化变量维度(在流形空间manifold的最小表示), Eigen::Vector3d: 优化变量类型
{
public:
...
public:
    ...
    {   // 计算误差函数，即计算$e = y - \hat{y}$，其中$\hat{y} = a^p + p^b - sin(cp) + e^{ap^2 - bp + \frac{c}{p}} - ln(dp+e)$
        ...
        const Eigen::Matrix<double, 5, 1> x = v->estimate(); // 顶点的优化变量
        _error(0, 0) = _measurement - (pow(x(0, 0), _p) + pow(_p, x(1, 0)) - sin(x(2, 0) * _p) + exp(x(0, 0) * _p * _p - x(1, 0) * _p + x(2, 0) / _p) - log(x(3, 0) * _p + x(4, 0))); // 计算误差 $e = y - \hat{y}$
    }
    // 计算雅可比矩阵
    ...
    {   // 计算雅可比矩阵函数，即计算$\frac{\partial e}{\partial x}$
        ...
        _jacobianOplusXi[0] = _p * pow(x(0, 0), _p - 1) + pow(_p, 2) * exp(x(0, 0) * _p * _p - x(1, 0) * _p + x(2, 0) / _p); 
        ...
        _jacobianOplusXi[1] = pow(_p, x(1, 0)) * log(_p) - _p * exp(x(0, 0) * _p * _p - x(1, 0) * _p + x(2, 0) / _p);
        ...
        _jacobianOplusXi[2] = -_p * cos(x(2, 0) * _p) + 1 / _p * exp(x(0, 0) * _p * _p - x(1, 0) * _p + x(2, 0) / _p);
        ...
        _jacobianOplusXi[3] = -_p / (x(3, 0) * _p + x(4, 0));
        ...
        _jacobianOplusXi[4] = -1 / (x(3, 0) * _p + x(4, 0));
        // cout<<_jacobianOplusXi[0]<<','<<_jacobianOplusXi[1]<<','<<_jacobianOplusXi[2]<<','<<_jacobianOplusXi[3]<<','<<_jacobianOplusXi[4]<<endl;
...
{
    // 生成数据
    double a_gt = 1.0, b_gt = -2.0, c_gt = 1.0, d_gt = 3.0, e_gt = 4.0; // 真实参数值
    double a_est = a_gt + 0.1, b_est = b_gt + 0.1, c_est = c_gt + 0.1, d_est = d_gt + 0.1, e_est = e_gt + 0.1; // 估计参数值
    int N = 100; // 数据点个数
    ...
    {
        double p = 1+ i / 100.0; // p值
        p_data.push_back(p); // p值存入数据容器
        y_data.push_back(pow(a_gt, p) + pow(p, b_gt) - sin(c_gt * p) + exp(a_gt * p * p - b_gt * p + c_gt / p) - log(d_gt * p + e_gt) + rng.gaussian(w_sigma * w_sigma)); // y值存入数据容器
        // cout<<p<<','<<y_data[i]<<endl;
    }
    //g2o编程流程
    //step1: 设置BlockSolver的优化变量维度和误差值维度
    typedef g2o::BlockSolver<g2o::BlockSolverTraits<5, 1>> Block; // 5: 优化变量维度, 1: 误差值维度
    ...
    Eigen::Matrix<double, 5, 1> x; // 优化变量
    x << a_est, b_est, c_est, d_est, e_est; // 优化变量赋值
    v->setEstimate(x); // 设置顶点的初始值
    v->setId(0); // 设置顶点的id
    ...
    //step8: 输出优化值
    Eigen::Matrix<double, 5, 1> x_estimate = v->estimate(); // 优化值
    cout << "estimated model: " << x_estimate.transpose() << endl; // 输出优化值
    return 0; // 程序结束
}
